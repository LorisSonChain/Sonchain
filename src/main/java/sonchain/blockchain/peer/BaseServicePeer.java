...
import owchart.owlib.Sock.Servers;

public class BaseServicePeer {
	public BaseServicePeer() {
	}
	...
	private HashMap<Integer, MessageListener> m_listeners = new HashMap<Integer, MessageListener>();
	
	private static int m_requestID = 10000;
	private static ArrayList<BaseServicePeer> m_services = new ArrayList<BaseServicePeer>();
	private HashMap<Integer, CMessage> m_waitMessages = new HashMap<Integer, CMessage>();
	...
	public static long GetDownFlow() {
		return BaseServicePeer.m_downFlow;
	}

	public static void SetDownFlow(long value) {
		BaseServicePeer.m_downFlow = value;
	}
	...
	public static int ConnectToServer(int proxyType, String ip, int port, String proxyIp, int proxyPort,
		...
		Clients.SetListener(new PeerSocketListener());
	...
	public static void AddService(BaseServicePeer service) {
	...
	public static void CallBack(int socketID, int localSID, byte[] str, int len) {
		...
		try {
			if (len > 4) {
				...
				BaseServicePeer service = null;
				for (BaseServicePeer sv : m_services) {
	...
	public static void WriteLog(int socketID, int localSID, int state, String log) {
		if (state == 2) {
			for (BaseServicePeer service : m_services) {
