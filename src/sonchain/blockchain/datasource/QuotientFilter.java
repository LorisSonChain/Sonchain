...
public class QuotientFilter implements Iterable<Long> {
	...
	public synchronized byte[] serialize() {
		...
		ret[2] = ELEMENT_BITS;
		System.arraycopy(ByteUtil.longToBytes(INDEX_MASK), 0, ret, 3, 8);
		System.arraycopy(ByteUtil.longToBytes(REMAINDER_MASK), 0, ret, 11, 8);
		System.arraycopy(ByteUtil.longToBytes(ELEMENT_MASK), 0, ret, 19, 8);
		System.arraycopy(ByteUtil.longToBytes(MAX_SIZE), 0, ret, 27, 8);
		System.arraycopy(ByteUtil.longToBytes(MAX_INSERTIONS), 0, ret, 35, 8);
		ret[43] = (byte) (overflowed ? 1 : 0);
		System.arraycopy(ByteUtil.longToBytes(entries), 0, ret, 44, 8);
		for (int i = 0; i < table.length; i++) {
			System.arraycopy(ByteUtil.longToBytes(table[i]), 0, ret, 52 + i * 8, 8);
		}
		return ret;
	}
	...
	public static QuotientFilter create(long largestNumberOfElements, long startingElements) {
		...
		/**
		 * The way sizing a quotient filter works is that the quotient bits +
		 * remainder bits is the maximum number of elements the filter can store
		 * before it runs out of fingerprint bits and can no longer be resized.
		 */
		...
		// I am pretty sure that even when completely full you want a non-zero
		// number of remainder bits
		// This also gives some emergency slack where even if you guess largest
		// number of elements wrong it will
	...
	public synchronized void insert(long hash) {
		...
		if (entries >= MAX_INSERTIONS | overflowed) {
			...
			if (REMAINDER_BITS > 1) {
				...
				// The filter can't accept more inserts and is effectively
				// broken
				overflowed = true;
				throw new OverflowedError();
			}
	...
	void deleteEntry(long s, long quot) {
		...
		while (true) {
			...
			if (isElementEmpty(next) | isElementClusterStart(next) | sp == orig) {
				...
				setElement(s, curr_occupied ? setElementOccupied(updated_next) : clearElementOccupied(updated_next));
				s = sp;
	...
	public synchronized void remove(long hash) {
		...
		if (!isElementOccupied(T_fq) | entries == 0) {
			// If you remove things that don't exist it's possible you will
			// clobber
	...
	// throw new IllegalArgumentException("All filters must have the same size
	// fingerprint");
	// }
	//
	// long totalEntries =
	// filters.stream().collect(Collectors.summingLong(filter ->
	// filter.entries));
	// int requiredQuotientBits =
	// bitsForNumElementsWithLoadFactor(totalEntries);
	...
	//
	// if (remainderBits < 1) {
	// throw new IllegalArgumentException("Impossible to merge not enough
	// fingerprint bits");
	// }
	//
	// QuotientFilter resultFilter = new QuotientFilter(requiredQuotientBits,
	// remainderBits);
	//
	// Iterable<QFIterator> iterators = (Iterable) filters.stream().map(filter
	// -> filter.iterator()).collect(Collectors.toList());
	// Iterator<Long> mergeQFIterator = Iterators.mergeSorted(iterators,
	// Ordering.natural());
	...
	/*
	 * Resizes the filter return a filter with the same contents and space for
	 * the minimum specified number of entries. This may allocate a new filter
	 * or return the existing filter.
	 */
	...
	// }
	//
	// sb.append(String.format("| is_shifted | is_continuation | is_occupied |
	// remainder"
	// + " nel=%d\n", entries));
	//
	...
	// sb.append(String.format("%d | ", isElementShifted(elt) == false ? 0 :
	// 1));
	// sb.append(String.format("%d | ", isElementContinuation(elt) == false ? 0
	// : 1));
	// sb.append(String.format("%d | ", isElementOccupied(elt) == false ? 0 :
	// 1));
