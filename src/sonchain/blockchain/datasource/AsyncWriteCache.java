...
public abstract class AsyncWriteCache<Key, Value> 
	...
	public static final Logger m_logger = Logger.getLogger(AsyncWriteCache.class);
    ...
    public long estimateCacheSize() {
        m_logger.debug("AsyncWriteCache estimateCacheSize start");
        ...
        return (long) (m_curCache.estimateCacheSize() * 2.0);
    }
    ...
    public synchronized void flipStorage() throws InterruptedException {
        ...
        try (CLock l = m_wLock.lock()) {
        	m_flushingCache.m_cache = m_curCache.m_cache;
            m_curCache = CreateCache(m_flushingCache);
        }
