...
public class Sign {
	...
	public static BigInteger recoverFromSignature(int recId, ECDSASignature sig, byte[] message) {
		...
		// the conversion
		// routine specified in Section 2.3.7, where mlen = ⌈(log2 p)/8⌉ or mlen
		// = ⌈m/8⌉.
		// 1.3. Convert the octet string (16 set binary digits)||X to an
		// elliptic curve point R
		// using the conversion routine specified in Section 2.3.4. If this
		// conversion
		...
		// + e = 0 (mod n).
		// In the above equation ** is point multiplication and + is point
		// addition (the EC group
		// operator).
		//
		// We can find the additive inverse by subtracting e from zero then
		// taking the mod. For
		// example the additive inverse of 3 modulo 11 is 8 because 3 + 8 mod 11
		// = 0, and
		// -3 mod 11 = 8.
	...
	/**
	 * Given an arbitrary piece of text and an Ethereum message signature
	 * encoded in bytes, returns the public key that was used to sign it. This
	 * can then be compared to the expected public key to determine if the
	 * signature was correct.
	 *
	 * @param message
	 *            RLP encoded message.
	 * @param signatureData
	 *            The message signature components
